...
export function createAcpBridge(options = {}) {
  ...
  async function promptWithWatchdog(remote, task, cwd, ctx) {
    ...
    let killCount = 0
    // v0.5.5：权限等待人类决策期间，把"等待时间"视为活跃（不触发冻结/超时）；
    // virtualLast 记录最近一次"因权限挂起而豁免"的检查时刻，决策落定后窗口从此刻重算。
    let virtualLast = 0
    const permPendingNow = () => {
      try { return typeof options.isPermissionPending === 'function' && options.isPermissionPending(remote.sessionId) } catch { return false }
    }
    for (;;) {
      ...
      const watchdogTimeout = new Promise((_, reject) => {
        const check = () => {
          ...
          const now = Date.now()
          // v0.5.5：权限请求正等待人类决策 → 视为活跃（虚拟心跳），不触发冻结/空闲超时
          if (permPendingNow()) {
            virtualLast = now
