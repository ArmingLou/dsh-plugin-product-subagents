...
export function apply(ctx, config = {}) {
  ...
  const isPermissionPending = (sessionIdOrThreadId) => {
    ...
    for (const [cid, rec] of bindings.entries()) {
      ...
      if (remoteId === sessionIdOrThreadId && pendingDecisions.has(cid)) return true
    }
    return false
  }
  ...
  const permissionHandler = async ({ product, sessionId, description, paths }) => {
    try {
      ...
      if (isRoundRejected(childId, paths || [])) {
      ...
      if (button !== null) {
        ...
        if (button === 'deny') addRoundRejected(childId, paths || [])
      ...
      if (outcome === 'rejected') {
        ...
        addRoundRejected(childId, paths || [])
        return 'deny'
      }
  ...
  const bridges = Object.fromEntries(
    Object.entries(providers)
      ...
      .map(([name, def]) => [name, createBridgeFor(def, { permissionHandler, isPermissionPending })]),
  )
  ...
  const scheduleDispose = (childId) => {
    ...
    const timer = setTimeout(() => {
      ...
      if (record) {
        ...
        const pending = remoteId && isPermissionPending(remoteId)
        if (pending) {
