...
export function registerProductDelegate(ctx, deps) {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      model: { type: 'string', description: 'Product model override. Omit to inherit the product\'s own default model configuration (claude-code: --model, codex: -c model=). ACP: applied per-session via setSessionConfigOption when the agent accepts the value, else the agent\'s own configuration.' },
      reasoning_effort: { type: 'string', enum: ['low', 'medium', 'high'], description: 'Product reasoning effort. Omit to inherit the product\'s own default (claude-code: --effort, codex: -c model_reasoning_effort=). ACP: best-effort via the session "effort" config option when the agent\'s model declares effort levels; otherwise ignored with a warning.' },
